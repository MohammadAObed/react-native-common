...
export function createContextCommon<T>(defaultValue?: T) {
  const context = createContextOrig<ContextValue<T> | undefined>(undefined);
  const ProviderOrig = context.Provider;

  const Provider = ({ value, children }: ContextProviderProps<T>): JSX.Element => {
  ...
  context.Provider = Provider as ProviderExoticComponent<ProviderProps<ContextValue<T> | undefined>>;

  return context;
}

export function useContextSelector<T, R>(context: React.Context<ContextValue<T> | undefined>, selector: (value: T) => R): R {
