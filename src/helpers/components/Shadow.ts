...
export function getContainerStyleFromChild(child: any) : ViewStyle {
  return  {width: child.props?.width ?? child.props?.style?.width ?? (child.props?.style as any[])?.vMax(y => y?.width)}
}
...
export function getStyleFromChild(child: any) : ViewStyle {
  return  {marginHorizontal: child.props?.marginHorizontal ?? child.props?.style?.marginHorizontal ?? (child.props?.style as any[])?.vMax(y => y?.marginHorizontal)}
}

export function isBlurSupported() {
  return Device.osName === 'Android' && parseInt(Device.osVersion, 10) >= 12;
} 
