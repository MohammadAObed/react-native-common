...
import type {
  PropertyMetaData,
  SetClassMetaDataParam,
  SetPropertyMetaDataParam,
} from '../../types/constants';

export function SetClassMetaData(metaData: SetClassMetaDataParam) {
  return function (_constructor: Function) {
    if (!PropertyMetaDataDictionary[metaData.className])
      PropertyMetaDataDictionary[metaData.className] = {};
  };
}
...
export function SetPropertyMetaData(metaData: SetPropertyMetaDataParam) {
  return function (_target: any, propertyKey: any) {
    const newMetaData: PropertyMetaData = {
      ...
      columnNameOfParentInOtherTable:
        metaData.columnNameOfParentInOtherTable ?? '',
    };

    if (!PropertyMetaDataDictionary[newMetaData.className])
      PropertyMetaDataDictionary[newMetaData.className] = {};
    if (!PropertyMetaDataDictionary[newMetaData.className]![propertyKey])
      PropertyMetaDataDictionary[newMetaData.className]![propertyKey] =
        newMetaData;
  };
}
...
export function GetPropertiesMetaData(
  className: string,
  throwError: true
): Record<string | number | symbol, PropertyMetaData>;
export function GetPropertiesMetaData(
  className: string,
  throwError?: false
): Record<string | number | symbol, PropertyMetaData> | undefined;
export function GetPropertiesMetaData(
  className: string,
  throwError = false
): Record<string | number | symbol, PropertyMetaData> | undefined {
  if (!PropertyMetaDataDictionary[className]) {
    if (throwError)
      throw new ErrorCustom(
        `Found no class in property meta data with name: ${className}`
      );
  }
...
export function GetPropertyMetaData(
  ...
  if (!PropertyMetaDataDictionary[className]?.[propertyName]) {
    if (throwError)
      throw new ErrorCustom(
        `Found no property in property meta data with class name: ${className}, property name: ${propertyName}`
      );
  }
  return PropertyMetaDataDictionary[className]?.[propertyName];
}

export function GetPropertyMetaDataFkClassName(
  className: string,
  propertyName: string,
  throwError = false
) {
  ...
  if (!fkClassName)
    throw new ErrorCustom(
      `No fKClassName Found with class name: ${className}, property name: ${propertyName}`
    );
  return fkClassName;
}
