...
export type Params<T extends undefined | ((...args: any) => any)> = T extends (...args: [infer P]) => any ? P : Parameters<NonNullable<T>>;
...
type EveryPropertyPathOf<Obj, Seen = never> = Obj extends object
  ? {
      [K in keyof Obj]: K extends Seen
        ? never
        :
            | (K extends UnWantedPropertiesIDKWhyTheyAppear ? never : K & string) /* | NonNullable<K> */
