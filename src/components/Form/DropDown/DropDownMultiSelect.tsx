...
import type { DropDownMultiProps, DropDownSingleAndMultiProps, DropDownValueField } from "../../../types/components";
...
export const DropDownMultiSelect = <Model,>({
  ...
  renderItemProps,
  ...rest
}: MultiSelectProps<Model> & DropDownMultiProps & DropDownSingleAndMultiProps<Model>) => {
  const { styles } = useStyles(getDropDownCustomStyles);

  let selectedItems = data.filter((x) => value?.includes(x[valueField]?.toString() ?? ""));

  let inputText: string | null = text ?? selectedItems.map((x) => x[labelField]).join(seperator);
    ...
    <MultiSelect
      ...
      onChange={(allValues) => {
        ...
        if (value && value.length > 0) {
          const unSelectValues = allValues.mGetDuplicates();
          allValues = allValues.filter((item) => !unSelectValues.includes(item));
        }
        onChange(allValues);
      }}
