...
export const FormRow = forwardRef(
  (
    { children, label, style, mode = 'normal', ...rest }: FormRowProps,
    ...
    return (
      <View
        ref={ref}
        style={[styles.formRowContainer, style, modeStyles[mode]]}
        {...rest}
      >
        {!isValidComponent(label) && label !== undefined && (
          <FormRowText>{label}:</FormRowText>
        )}
        {isValidComponent(label) && label !== undefined && (
