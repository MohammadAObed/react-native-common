...
export const ScrollContainer = forwardRef(({ children }: ScrollViewProps, ref: React.ForwardedRef<ScrollView>) => {
  const { styles } = useStyles(getScrollContainerStyles);
  return (
    <ScrollView ref={ref} style={styles.container} contentContainerStyle={styles.contentContainer}>
      {children}
    </ScrollView>
  );
});
