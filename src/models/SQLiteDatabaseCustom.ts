...
import {
  getNewestId,
  GetPropertyMetaData,
  GetPropertyMetaDataFkClassName,
} from '../helpers';
...
export class SQLiteDatabaseCustom<ClassNames extends string = string> {
  ...
  static async migrateDbIfNeeded(
    ...
    handleDbVersion: HandleDbVersion
  ) {
  ...
  async handleDbVersionCommon(
    currentDbVersion: number,
    getSqlFile: GetSqlFile
  ) {
    const [sqlFile] = await getSqlFile(currentDbVersion);
    if (!sqlFile)
      throw new ErrorCustom(DB_ERROR_MESSAGE, ErrorCode.LOAD_DB_ASSET);
    if (!sqlFile.localUri)
      throw new ErrorCustom(
        DB_ERROR_MESSAGE,
        ErrorCode.LOAD_DB_ASSET_LOCAL_URI
      );
  ...
  async handleGetAsync<T>(cb: () => T, source: string) {
    ...
    if (
      !source.toLowerCase().includes('sqlite_master') &&
      match &&
      match.length > 1
    ) {
      console.log('=============3============');
      ...
      const [booleans, result] = await Promise.all([
        this.getBooleanTableConstraintsAsync(tableName),
        cb(),
      ]);
  ...
  handleGetSync<T>(cb: () => T, source: string) {
    ...
    if (
      ...
      const tableName = match[1] as ClassNames;
      const [booleans, result] = [
        this.getBooleanTableConstraintsSync(tableName),
        cb(),
      ];
      console.log(
        '🚀 ~ SQLiteDatabaseCustom<ClassNamesValues ~ handleGetSync ~ booleans:',
        booleans
      );
      this._handleValueFromDb(result as any, booleans);
      return result;
  ...
  getFirstAsync = async <T>(
    ...
    const result = await this.handleGetAsync(
      async () => await this._db.getFirstAsync<T>(source, ...params),
      source
    );
    return result;
  };
  getFirstSync = <T>(source: string, ...params: SQLiteVariadicBindParams) => {
    const result = this.handleGetSync(
      () => this._db.getFirstSync<T>(source, ...params),
      source
    );
    return result;
  };
  getAllAsync = async <T>(
    ...
    console.log('=============1============');
    const result = await this.handleGetAsync(
      async () => await this._db.getAllAsync<T>(source, ...params),
      source
    );
    return result;
  };
  getAllSync = <T>(
    ...
    const result = this.handleGetSync(
      () => this._db.getAllSync<T>(source, ...params),
      source
    );
    return result;
  };
  ...
  runAsync = async (source: string, ...params: SQLiteVariadicBindParams) =>
    await this._db.runAsync(source, params);
  runSync = (source: string, ...params: SQLiteVariadicBindParams) =>
    this._db.runSync(source, params);
  withExclusiveTransactionAsync = async (
    task: (txn: TransactionCustom<ClassNames>) => Promise<void>
  ) =>
  ...
  async validateAsync() {
    if (!(await this.isInTransactionAsync()))
      throw new ErrorCustom(
        ...
        ErrorCode.NO_DB_TRANSACTION_FOUND
      );
  }
  validateSync() {
    if (!this.isInTransactionSync())
      throw new ErrorCustom(
        'Failed to process resources',
        ErrorCode.NO_DB_TRANSACTION_FOUND
      );
  ...
  async getByIdAsync<T>(tableName: ClassNames, Id: number) {
    return await this.getFirstAsync<T>(
  ...
  async getForeignKeysAsync(tableName: ClassNames) {
    return await this.getAllAsync<SqliteForeignKey>(
      `PRAGMA foreign_key_list(${tableName})`
    );
  }
  getForeignKeysSync(tableName: ClassNames) {
    return this.getAllSync<SqliteForeignKey>(
      `PRAGMA foreign_key_list(${tableName})`
    );
  }
  async getTableInfoAsync(tableName: ClassNames) {
    return await this.getAllAsync<SqliteTableColumnInfo>(
      `PRAGMA table_info(${tableName})`
    );
  }
  getTableInfoSync(tableName: ClassNames) {
    return this.getAllSync<SqliteTableColumnInfo>(
      `PRAGMA table_info(${tableName})`
    );
  }
  async getTableConstraintsAsync(tableName: ClassNames) {
    ...
    if (!result)
      throw new ErrorCustom(`no constraints found for table: ${tableName}`);
    if (!result.sql)
      throw new ErrorCustom(`no sql constraint found for table: ${tableName}`);
    console.log('===========4.2===========result', result);
    return this._extractCheckConstraints(result.sql);
  }
  getTableConstraintsSync(tableName: ClassNames) {
    console.log(
      '🚀 ~ SQLiteDatabaseCustom<ClassNamesValues ~ getTableConstraintsSync ~ tableName:',
      tableName
    );
    ...
    if (!result.sql)
      throw new ErrorCustom(`no sql constraint found for table: ${tableName}`);
    console.log(
      '🚀 ~ SQLiteDatabaseCustom<ClassNamesValues ~ getTableConstraintsSync ~ result:',
      result
    );
    return this._extractCheckConstraints(result.sql);
  }
  ...
  async add(
    ...
    const { columns, props } = await this._handleAddOrUpdateData(
      tableName,
      record
    );
    console.log('============6.11.2.2============columns: ', columns);
    const FKs = (await this._handleFKs(tableName, columns)).filter(
      (fk) => nestedIds[fk.table as ClassNames]
    );
    ...
    let values = [
      ...columns.map((x) => this._handleValueToDb(x[1])),
      ...FKs.map((fk) => nestedIds[fk.table as ClassNames]),
    ].join(', ');
    console.log(
      '============6.11.2.5============: ',
  ...
  async update(
    ...
    // console.log("============5============");
    const result = await this.runAsync(
      `UPDATE ${tableName} SET ${entries} WHERE Id = ?`,
      record.Id
    );
    console.log('============6============');
    await this._handleNested(
      tableName,
      record,
      props,
      nestedIds,
  ...
  async addOrUpdate(
    ...
    record: Common,
    nestedIds: NestedIds<ClassNames> = {},
    _deleteMissingNested = true
  ) {
    console.log('============6.11.1============');
    await this.validateAsync();
    let dbRecord = await this.getByIdAsync(tableName, record.Id);
    console.log('============6.11.2============', dbRecord);
    const result = await (dbRecord
      ? this.update(tableName, record, nestedIds)
      : this.add(tableName, record, nestedIds));
    console.log('============6.11.3============');
  ...
  protected _handleValueFromDb<T extends Common>(
    result: T | T[] | null,
    booleans: string[]
  ) {
  ...
  private async _handleAddOrUpdateData(tableName: ClassNames, record: Common) {
    const props = getEntries(record).filter(
      (x) => !GetPropertyMetaData(tableName, x[0])?.DbIgnore
    );
  ...
  private async _handleFKs(tableName: ClassNames, columns: [string, any][]) {
    ...
    const wantedFKs = FKs.filter(
      (fk) => !columns.some((x) => x[0] === fk.from)
    );
    return wantedFKs;
  }
  private async _handleNested(
    ...
    const fKObjects = props.filter(
      (x) => GetPropertyMetaData(tableName, x[0])?.isFKObject
    );
    console.log('============6.2============');
    for (let fkObject of fKObjects) {
      const fKClassName = GetPropertyMetaDataFkClassName(
        tableName,
        fkObject[0]
      );
      if (!fKClassName)
        throw new ErrorCustom(
          `No fKClassName Found with class name: ${tableName}, property name: ${fkObject[0]}`
        );
      console.log('============6.3============');
    ...
    const fKArrays = props.filter(
      (x) => GetPropertyMetaData(tableName, x[0])?.isFKArray
    );
    console.log('============6.5============');
    for (let fkArray of fKArrays) {
      ...
      if (
        deleteMissingNested &&
        !fkArray[1].some((x: unknown) => isPrimitive(x))
      ) {
        await this.deleteNestedMissing(
          fKClassName,
      ...
      for (let fkArrayItem of fkArray[1]) {
        ...
        if (isPrimitive(fkArrayItem)) {
          console.log('============6.9============');
          await this._handleFKPrimitiveArray(
            fkArrayItem,
            GetPropertyMetaData(tableName, fkArray[0], true),
            nestedIds
          );
          console.log('============6.10============');
        } else {
  ...
  private async deleteNestedMissing(
    tableName: ClassNames,
    Ids: number[],
    ParentTableName: ClassNames,
    parentId: number
  ) {
    if (!parentId || !Ids || Ids.length === 0) return;
    const propName = (await this.getForeignKeysAsync(tableName)).find(
      (x) => x.table === ParentTableName
    )?.from;
    if (!propName) return;
    console.log(
      '============0.0.0============query:',
  ...
  private async _handleFKPrimitiveArray(
    value: unknown,
  ...
  private async _deleteXTable(
    metaData: PropertyMetaData,
    nestedIds: NestedIds<ClassNames>
  ) {
    const FKs = (await this.getForeignKeysAsync(metaData.fKClassName)).filter(
      (x) => x.from !== metaData.columnNameInOtherTable
    );
    const OldRecordWhereCondition = FKs.map(
      (x) => `${x.from} = ${nestedIds[x.table as ClassNames]}`
    ).join(' AND ');
    await this.runAsync(
      `DELETE FROM ${metaData.fKClassName} WHERE ${OldRecordWhereCondition}`
    );
  }
  private _extractCheckConstraints(sql: string): Record<string, string> {
    ...
    while ((match = checkRegex.exec(sql)) !== null) {
      ...
      checkColumns[columnName] = checkCondition.slice(
        0,
  ...
  private async _setIdTracker() {
    ...
    const tableNames = tablesResult
      .map((x) => `(SELECT ${minOrMax}(Id) FROM ${x.name}) AS ${x.name}`)
      .join(', ');
    const query = `SELECT ${tableNames}`;
    const result =
      (await this.getFirstAsync<Record<string, number>>(query)) ?? {};
    NewIdsTracker.push(
  ...
  private async _getDbVersion() {
    let result = await this.getFirstAsync<{ user_version: number }>(
      'PRAGMA user_version'
    );
    if (!result)
      throw new ErrorCustom(DB_ERROR_MESSAGE, ErrorCode.GET_DB_VERSION);
    return result;
  }
...
export class TransactionCustom<
  ClassNamesValues extends string,
> extends SQLiteDatabaseCustom<ClassNamesValues> {
